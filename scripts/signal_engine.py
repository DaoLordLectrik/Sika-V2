...
def generate_signal_from_row(row: pd.Series, 
    ...
    if direction != "HOLD" and price is not None and atr is not None and not pd.isna(atr):
        if direction == "BUY":
            stop_loss = price - (sl_atr_multiplier * atr)
            take_profit = price + (tp_atr_multiplier * atr)
        else:  # SELL
            stop_loss = price + (sl_atr_multiplier * atr)
            take_profit = price - (tp_atr_multiplier * atr)
